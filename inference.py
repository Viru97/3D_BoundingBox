...
def run_sample(model, device, sample_dir, out_dir, sample_idx, total,
    ...
    for i, mask in enumerate(masks):
        ...
        # Robust outlier removal — MAD has 50% breakdown point vs IQR's 25%
        ...
        # Median anchor — robust to any remaining outliers
    ...
    # ── robust point cloud cleaning for visualisation ──────────────────────
    # IQR breaks when >25% of pixels are outliers.
    # MAD (median absolute deviation) has a 50% breakdown point.
    pc_flat = pc.reshape(3, -1)
    valid   = pc_flat[2] > 0.01              # drop zero/missing depth
    pc_vis  = pc_flat[:, valid].copy()

    if pc_vis.shape[1] > 100:
        med = np.median(pc_vis, axis=1, keepdims=True)          # (3,1)
        mad = np.median(np.abs(pc_vis - med), axis=1,
                        keepdims=True) + 1e-6                    # (3,1)
        inlier = np.all(np.abs(pc_vis - med) < 5.0 * mad, axis=0)
        if inlier.sum() > 50:
            pc_vis = pc_vis[:, inlier]

    n_sub = max(1, pc_vis.shape[1] // 100)
    sub   = np.random.choice(pc_vis.shape[1], n_sub, replace=False)
    pc_sub = pc_vis[:, sub]                  # (3, n_sub)  — consistent array
    ax3.scatter(pc_sub[0], pc_sub[1], pc_sub[2],
                s=0.5, c=pc_sub[2], cmap="viridis", alpha=0.4)
    ...
    # ── lock axes to GT box extent so bad predictions don't blow up the view ──
    ref_boxes = list(gt_boxes) if len(gt_boxes) else (
                predicted_boxes if predicted_boxes else None)
    if ref_boxes is not None:
        all_c  = np.concatenate([np.array(b) for b in ref_boxes], axis=0)
        margin = 0.25
        ax3.set_xlim(all_c[:,0].min()-margin, all_c[:,0].max()+margin)
        ax3.set_ylim(all_c[:,1].min()-margin, all_c[:,1].max()+margin)
        ax3.set_zlim(all_c[:,2].min()-margin, all_c[:,2].max()+margin)
